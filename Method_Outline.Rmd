...
We have saved the nearest neighbours within a 50 km radius of each station. See the helper file, `helper_get_nbrs.R`.
...
4. Estimate Correlation (TUESDAY)

We only use nearest neighbour infilling if the neighbours are highly correlated. We estimate the correlation using spearmans rank correlation, given rainfall is skew and non-linear.

```{r}
1+1
```

5. Add Reconstruction Column Data (WEDNESDAY)

6. Get Maxima (raw and reconstructed) (THURSDAY)

## 7. Run Viney Bates on raw data (MONDAY)

To check for untagged accumulations present amongst daily observations we use a test given in Viney and Bates (2004).
